...
from prophecy.utils import *
from sw_create_embeddings.graph import *
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/sw_create_embeddings")
    registerUDFs(spark)
    
    MetricsCollector.start(spark = spark, pipelineId = "pipelines/sw_create_embeddings")
    pipeline(spark)
